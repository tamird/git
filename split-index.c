...
{
	struct split_index *si = istate->split_index;
	struct cache_entry *first_new, *last_base;
	...
	/*
	 * add_index_entry() appends in constant time when the new name sorts
	 * after the last existing name. Keep that path for append-only split
	 * indexes instead of scanning the whole base.
	 */
	if (!istate->cache_nr)
		goto insert_entries;
	first_new = si->saved_cache[si->nr_replacements];
	last_base = istate->cache[istate->cache_nr - 1];
	if (!ce_same_name(last_base, first_new) &&
