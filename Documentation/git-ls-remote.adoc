...
--------
[verse]
'git ls-remote' [--branches] [--tags] [--refs] [--exact-ref]
	      [--upload-pack=<exec>]
...
--exact-ref::
	When listing refs, require at least one <patterns> argument and
	treat each one as a complete refname under `refs/` or `HEAD`.
	Match the full name rather than a tail or glob pattern. Abbreviated
	names and wildcards are rejected. With protocol version 2, each name
	is sent as a ref-prefix to limit the remote's advertisement; refs with
	the same prefix but a different name are filtered locally.
	Older protocols send the full advertisement, which Git filters locally.
	An annotated tag's peeled form is also shown unless
	`--refs` is given. With `--get-url`, no refs are listed and patterns
	are ignored.
...
<patterns>...::
	...
	`refs/heads/foobar`).
	With `--exact-ref`, each argument is a complete refname as described
	above.
