...
commitGraph.changedPathsVersion::
	...
	write. May be -1, 0, 1, 2, 3, or 4. Note that values greater than 1 may be
...
Bloom filters.
+
If 3, Git will only read version 3 Bloom filters, and will write version 3
Bloom filters. Version 3 also records path basenames so that leading `**/`
pathspecs with a literal basename can use Bloom filters.
+
If 4, Git will only read version 4 Bloom filters, and will write version 4
Bloom filters. Version 4 separates basename keys from full-path keys and
uses an odd double-hash step to avoid repeated bit positions among the
default seven probes. Existing filters must be recomputed to use version 4.
+
