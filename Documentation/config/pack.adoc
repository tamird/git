...
pack.useAggressiveShallowEdges::
	When true, thin packs for shallow repositories spend additional
	time finding excluded objects to reduce the resulting pack size.
	When false, pack generation may be faster at the cost of sending
	additional objects. Default is `true`.
