...
`fetch.packfileUriJobs`::
	Specifies the maximum number of concurrent packfile URI downloads.
	The default is 4. A value of 1 fetches and indexes URI packs one
	at a time.
+
With values greater than 1, URI packs are indexed in advertised order
after downloading. When this setting is explicitly configured to a value
greater than 1 and the server advertises the `no-ref-delta` promise, the
client requests it and may index multiple URI packs concurrently, each
checked with `index-pack --no-ref-delta`.
