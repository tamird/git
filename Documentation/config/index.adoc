...
index.skipHash::
	When enabled, do not compute the trailing hash for ordinary index
	files. This accelerates Git commands that manipulate the index, such
	as `git add`, `git commit`, or `git status`. Instead of storing the
	checksum, write a trailing set of bytes with value zero, indicating
	that the computation was skipped. Shared index files are still hashed
	because their trailing hash identifies the file.
+
