...
maintenance.geometric-repack.auto::
	...
	there are eligible packfiles that need to be merged together to retain the
	...
	6700.
+
When `core.multiPackIndex` is enabled, geometric repacks use incremental
multi-pack-index layers and honor `repack.midxNewLayerThreshold` when
selecting packs. Full repacks and cruft cleanup still consider the
complete pack geometry.
