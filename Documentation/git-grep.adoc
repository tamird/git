...
`--content-index`::
`--no-content-index`::
	Use grep acceleration indexes, when available, to avoid reading blobs
	that cannot contain a case-sensitive literal pattern. This is the
	default. The indexes are created and updated by
	linkgit:git-grep-index[1].
