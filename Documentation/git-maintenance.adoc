...
geometric-repack::
	Combine packfiles according to the geometric progression described by
	`git repack --geometric`. With `core.multiPackIndex` enabled, geometric
	repacks write incremental multi-pack-index layers and retain packs
	protected by older layers or `repack.midxNewLayerThreshold`. The
	`--auto` check uses the same eligible packs. If the complete pack
	geometry calls for combining all packs, the task instead performs a
	full repack with cruft cleanup and writes a flat multi-pack-index.
