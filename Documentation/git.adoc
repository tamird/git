...
--no-fsmonitor-hook::
	Do not run an fsmonitor hook command named by `core.fsmonitor`.
	A non-Boolean value is rejected with the usual configuration error.
	Boolean `true` still selects the built-in file system monitor; Boolean
	`false` or an unset value disables fsmonitor.  Git subprocesses inherit
	the restriction, but it does not persist for independent invocations.
	This does not restrict commands selected by other configuration variables.
