...
+
With the `--recompute-truncated` option, recompute existing Bloom filters
that were replaced by an always-matching filter because they exceeded
the size limit. Requires `--changed-paths`, either explicitly or through
configuration. Other existing filters are reused. Use `--split=replace`
to rewrite earlier layers and `--append` to retain their commits even
when they are no longer reachable. This can be expensive: filters
that still exceed the limit are also recomputed, including on repeated
...
configuration.
+
Recomputations requested by `--recompute-truncated` also count against
this limit, even when the resulting filter is still truncated. Repeating
a limited write may retry the same truncated filters before reaching
others. The limit does not bound the work of rewriting graph metadata.
+
