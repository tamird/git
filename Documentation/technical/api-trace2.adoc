...
`"start"`::
	This event contains the complete argv received by main() and the
	initial working directory, before processing `-C` or repository setup.
	The `cwd` field is omitted if the working directory cannot be obtained.
+
------------
{
	"event":"start",
