...
spawns the workers and sends entries to them in round-robin order,
keeping a bounded number of entries queued to each worker. As workers
report results, the main process refills the queue of whichever worker
finished an entry. This lets faster workers take more of the remaining
work, while the bound limits how much work can be stranded behind a slow
entry and amortizes the per-item IPC round trip. Compared with continuous
chunks, this trades some path locality for better load balancing.
