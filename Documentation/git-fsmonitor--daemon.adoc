...
stop::
	...
	directory, if present. On macOS, this also stops monitoring
	the repository's other linked worktrees.
...
This daemon is a long running process used to watch a working
directory and maintain a list of the recently changed files and
directories.  Performance of commands such as `git status` can be
increased if they just ask for a summary of changes to the working
directory and can avoid scanning the disk.

On macOS, linked worktrees in the same repository share one daemon,
which maintains a separate list of changes for each working directory.
...
By default, the socket is created in the `.git` directory.  On macOS,
linked worktrees share a socket in their common `.git` directory.  If
that directory is on a network-mounted filesystem or its socket path
exceeds the platform's Unix-domain socket limit, the socket is instead created at
`$HOME/.git-fsmonitor-*` unless `$HOME` itself is on a network-mounted
filesystem.  In that case, set `fsmonitor.socketDir` to a directory on a
native filesystem.
