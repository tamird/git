...
By default, the socket is created in the `.git` directory.  If that directory
is on a network-mounted filesystem or its socket path exceeds the platform's
Unix-domain socket limit, the socket is instead created at
