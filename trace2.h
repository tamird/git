...
enum trace2_timer_id {
	...
	TRACE2_TIMER_ID_UNPACK_TREES_EXISTING_MATCH_REMOVE,
	...
	TRACE2_TIMER_ID_UNTRACKED_CACHE_REVALIDATE_IGNORE_ATTEMPT,
	...
	TRACE2_TIMER_ID_FETCH_PRUNE_REFS,
 ...
 */
#define TRACE2_FOLLOW_OID_SAMPLE_MODULUS 64
#define TRACE2_FOLLOW_OID_SAMPLE_MAX_DISTINCT 65536
...
enum trace2_counter_id {
	...
	/* Keep this checked, command-cumulative ODB family contiguous. */
	...
	/* Optional lookup phases have validity separate from the ODB totals. */
	...
	/* Checked sample of completed requested tree descriptor reads. */
	...
	TRACE2_COUNTER_ID_CACHE_TREE_FLAT_NODES,

	/* Keep this block contiguous for the final update DATA snapshot. */
	...
	/* Sampled graph-hit ODB checks in ref-filter's sorted preload. */
	...
	/* Sampled graph-hit ODB checks while marking local fetch tips complete. */
	...
	/* Subdivide the existing rename/populate timer without extra clocks. */
	...
	/* Directory traversal callers that initialize the index name hash. */
	...
	/* Reasons the sorted-index probe needs the name hash. */
...
/* Add consecutive counters with one thread-local counter-block lookup. */
