...
static struct untracked_cache_dir *find_untracked(struct untracked_cache_dir *dir,
						    ...
						    const char *name, int len)
{
	...
	if (!dir)
		return NULL;
	d = find_untracked(dir, name, len, &insert_pos);
	...
	MOVE_ARRAY(dir->dirs + insert_pos + 1, dir->dirs + insert_pos,
		   dir->dirs_nr - insert_pos);
	dir->dirs_nr++;
	dir->dirs[insert_pos] = d;
	return d;
}
					   ...
					   const struct pathspec *pathspec)
{
	...
	untracked_prune = find_untracked(untracked_prune,
					 dirname + baselen, len - baselen, NULL);
	...
	while (!read_cached_dir(&cdir)) {
		...
		if (state == path_recurse) {
			...
			ud = lookup_untracked(dir->untracked,
					      ...
					      path.buf + baselen,
					      path.len - baselen);
			pd = find_untracked(untracked_prune, path.buf + baselen,
					    path.len - baselen, NULL);
			subdir_state =
			 ...
			 struct write_data *wd, unsigned int depth)
{
	...
	if (wd->bounded_snapshot) {
		if (wd->index >= UNTRACKED_SNAPSHOT_MAX_DIRS) {
			*wd->bound = UNTRACKED_SNAPSHOT_BOUND_DIRS;
			return -1;
		}
		if (depth >= UNTRACKED_SNAPSHOT_MAX_DEPTH) {
			*wd->bound = UNTRACKED_SNAPSHOT_BOUND_DEPTH;
			return -1;
		}
	}
	i = wd->index++;
	...
	if (wd->bounded_snapshot) {
		if (untracked->untracked_nr > wd->remaining_entries ||
			...
			*wd->bound = UNTRACKED_SNAPSHOT_BOUND_ENTRIES;
			return -1;
		}
