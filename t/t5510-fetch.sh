...
test_expect_success CASE_INSENSITIVE_FS,REFFILES 'D/F conflict on case insensitive filesystem' '
	...
	(
		...
		test_must_fail git fetch -f origin "refs/heads/*:refs/heads/*" 2>err &&
		test_grep -F "cannot lock ref ${SQ}refs/remotes/origin/foo${SQ}: there is a non-empty directory ${SQ}$PWD/./refs/remotes/origin/foo${SQ} blocking reference ${SQ}refs/remotes/origin/foo${SQ}" err &&
		git rev-parse refs/heads/main >expect &&
		git rev-parse refs/heads/Foo/bar >actual &&
		test_cmp expect actual
	)
