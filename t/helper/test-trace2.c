...
static void follow_lookup_counter_sample(uint64_t ns)
{
	trace2_counter_add(TRACE2_COUNTER_ID_DIFF_FOLLOW_LOOKUP_LOCATION_COUNT, 1);
	trace2_counter_add(TRACE2_COUNTER_ID_DIFF_FOLLOW_LOOKUP_MIDX_SEARCH_COUNT, 1);
	trace2_counter_add(TRACE2_COUNTER_ID_DIFF_FOLLOW_LOOKUP_MIDX_SEARCH_NS, ns);
}

static void *follow_lookup_counter_thread(void *data)
{
	uint64_t *ns = data;

	trace2_thread_start("follow_lookup_counter");
	follow_odb_counter_sample(800);
	follow_lookup_counter_sample(*ns);
	trace2_thread_exit();
	return NULL;
}

static int ut_203follow_lookup_counter(int argc, const char **argv)
{
	pthread_t thread;
	uint64_t worker_ns;

	if (argc != 1)
		die("expect merged, increment-overflow, merge-overflow or outer-invalid");
	if (!strcmp(argv[0], "outer-invalid")) {
		follow_odb_counter_sample(UINT64_MAX);
		trace2_counter_add(TRACE2_COUNTER_ID_DIFF_FOLLOW_ODB_LOCATION_NS, 1);
		follow_lookup_counter_sample(800);
	} else if (!strcmp(argv[0], "increment-overflow")) {
		follow_odb_counter_sample(800);
		follow_lookup_counter_sample(UINT64_MAX);
		trace2_counter_add(TRACE2_COUNTER_ID_DIFF_FOLLOW_LOOKUP_MIDX_SEARCH_NS, 1);
	} else {
		follow_odb_counter_sample(800);
		if (!strcmp(argv[0], "merged")) {
			follow_lookup_counter_sample(800);
			...
			die("unknown follow packed-lookup counter mode: %s", argv[0]);
		}
		if (pthread_create(&thread, NULL, follow_lookup_counter_thread, &worker_ns))
			die("failed to create thread");
		if (pthread_join(thread, NULL))
			die("failed to join thread");
	}
