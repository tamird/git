...
test_expect_success 'setup history that changes the blamed file' '
	git switch --orphan changed-every-time &&
	test_commit_bulk --filename=changed \
		--contents="survivor\\nchange %s" --notick 16384 &&
	git commit-graph write --reachable --changed-paths
'

test_perf 'blame when the file changes in every commit' '
	git blame -L 1,1 changed >/dev/null
'
