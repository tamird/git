...
test_expect_success 'grep reuses observed worktree blob bytes' '
	test_when_finished "rm -f .git/fsmonitor-attributes \
		.git/fsmonitor-equal .git/index.before-rewrite \
	...
	test_grep "bad boolean config value .*invalid.*grep.worktreeblobcache" \
		err &&
	...
	git ls-files --stage >index-entries.before &&
	cp .git/index .git/index.before-rewrite &&
	test-tool chmtime +10 grep-worktree-equal &&
	git -c core.fsmonitor=false update-index --refresh &&
	git update-index --fsmonitor-valid grep-worktree-equal &&
	! cmp .git/index.before-rewrite .git/index &&
	rm .git/index.before-rewrite &&
	git ls-files --stage >index-entries.after &&
	test_cmp index-entries.before index-entries.after &&
	test_expect_code 1 env \
...
test_expect_success 'grep rejects stale cache after index replacement' '
	test_config grep.worktreeBlobCache true &&
	test_when_finished "rm -f .git/fsmonitor-replace-index \
		...
		grep-worktree-trace-race-rebuild \
		grep-worktree-trace-race-hit &&
		git update-index --no-fsmonitor &&
	...
	cp .git/index.original .git/index &&
	...
	test_trace2_data grep worktree_blob/recorded_equal 1 \
		<grep-worktree-trace-race &&
	test_expect_code 1 env \
		GIT_TRACE2_EVENT="$PWD/grep-worktree-trace-race-rebuild" \
		git grep "absent rebuilt sidecar" -- \
			grep-worktree-race-equal &&
	test_trace2_data grep worktree_blob/hits 0 \
		<grep-worktree-trace-race-rebuild &&
	test_trace2_data grep worktree_blob/recorded_equal 1 \
		<grep-worktree-trace-race-rebuild &&
	test_expect_code 1 env \
		GIT_TRACE2_EVENT="$PWD/grep-worktree-trace-race-hit" \
		git grep "absent rebuilt sidecar" -- \
			grep-worktree-race-equal &&
	test_trace2_data grep worktree_blob/hits 1 \
		<grep-worktree-trace-race-hit &&
