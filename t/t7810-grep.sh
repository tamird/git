...
test_expect_success LIBPCRE2 'POSIX anchors preserve matches' '
	...
	printf "\nmissing\n" >>anchor-lookahead &&
	cat >expect <<-\EOF &&
	anchor-literal:1:literal^anchor
	anchor-literal:2:literal$anchor
	anchor-literal:3:literal anchor
	anchor-lookahead:1:partial
	anchor-lookahead:2:partial suffix
	anchor-lookahead:3:prefix partial
	anchor-lookahead:4:prefix partial suffix
	anchor-lookahead:5:partial
	anchor-lookahead:6:missing
	EOF
	git grep --no-index --threads=1 -n -e "^missing" -e "^absent" \
		-e "partial" -e "literal" -e "n[oe]ver" \
...
test_expect_success 'grep reuses observed worktree blob bytes' '
	...
	test_grep ! "\"event\":\".*timer\".*\"name\":\"worktree-cache/" \
		grep-worktree-trace-textconv &&
	...
	replacement=$(
		echo "replacement worktree blob" |
		git hash-object -w --stdin
	) &&
	git update-index --cacheinfo 100644,$replacement,grep-worktree-equal &&
	git update-index --fsmonitor-valid grep-worktree-equal &&
	test_expect_code 1 env \
		GIT_TRACE2_EVENT="$PWD/grep-worktree-trace-oid-change" \
		git grep "absent worktree blob" -- grep-worktree-equal &&
	test_trace2_data grep worktree_blob/hits 0 \
		<grep-worktree-trace-oid-change &&
	git update-index \
		--cacheinfo 100644,$oid,grep-worktree-equal \
		grep-worktree-equal \
		--fsmonitor-valid grep-worktree-equal &&
	...
	then
		...
		test_trace2_data grep worktree_blob/recovery_write_outcome 5 \
			<grep-worktree-trace-refresh &&
		test_grep "\"event\":\"timer\".*\"category\":\"grep\",\"name\":\"worktree-cache/write\"" \
			grep-worktree-trace-refresh >actual &&
		test_line_count = 1 actual &&
		test_grep "\"intervals\":1," actual &&
		test_grep ! "\"event\":\"th_timer\".*\"name\":\"worktree-cache/write\"" \
			grep-worktree-trace-refresh &&
		test_grep ! "\"event\":\"region_.*\".*\"label\":\"worktree-cache/write\"" \
			grep-worktree-trace-refresh &&
		test_grep "\"event\":\"timer\".*\"category\":\"grep\",\"name\":\"worktree-cache/recovery-prepare\"" \
			grep-worktree-trace-refresh >actual &&
		test_line_count = 1 actual &&
		test_grep "\"intervals\":1," actual &&
		test_grep ! "\"event\":\"th_timer\".*\"name\":\"worktree-cache/recovery-prepare\"" \
			grep-worktree-trace-refresh &&
...
test_expect_success NO_FORCED_SPLIT_INDEX \
	...
	(
		...
		# Checksum validation is lazy. If the corrupted identity is
		# visited first, grep scans it before another hit selects the
		# valid fallback generation.
		if test_trace2_data grep worktree_blob/recovered_identity 2 \
