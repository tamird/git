...
test_expect_success '--follow falls back for rewritten parents' '
...
test_expect_success '--simplify-by-decoration keeps --follow side history' '
	git init follow-decoration &&
	(
		cd follow-decoration &&
		...
		git -c core.commitGraph=false log \
			--simplify-by-decoration --format=%s \
			--follow -- old >expect &&
		git -c core.commitGraph=true log \
			--simplify-by-decoration --format=%s \
			--follow -- old >actual &&

		test_grep -x side-change actual &&
		test_cmp expect actual
	)
'
...
test_expect_success 'version 3 factors multi-path Bloom queries' '
	...
	(
		...
		git -c commitGraph.changedPathsVersion=3 commit-graph write \
			--reachable --changed-paths &&
		git -c core.commitGraph=false log --format=%s -- \
			first/shared/lock second/shared/lock \
			":(exclude)second/shared/lock" >expect &&
