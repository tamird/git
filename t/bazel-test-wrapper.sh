...
repository="$(runfiles_current_repository 1 2>/dev/null || true)"
...
if ! git_bin="$(rlocation "${workspace}/git")"; then
  echo "missing runfile: ${workspace}/git" >&2
  exit 1
fi
if ! build_opts="$(rlocation "${workspace}/GIT-BUILD-OPTIONS")"; then
  echo "missing runfile: ${workspace}/GIT-BUILD-OPTIONS" >&2
  exit 1
fi
