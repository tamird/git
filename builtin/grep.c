			  ...
			  const char *filename, const char *path, int cached)
{
	...
	if (oid) {
		...
		hit = grep_tree(&subopt, pathspec, &tree, &base, base.len,
				object_type == OBJ_COMMIT, NULL, NULL, NULL);
		strbuf_release(&base);
		free(data);
	} else {
		      ...
		      int include_untracked, int use_exclude)
{
	...
	if (include_untracked) {
		...
		if (use_exclude) {
			...
			untracked_dir.untracked_cache_negative_only = 1;
	...
	if (include_untracked && !full_worktree && content_index_query) {
		...
		struct pathspec_index_range full_index = { 0, repo->index->cache_nr };
		struct pathspec_index_range *ranges = &full_index;
		...
		if (min_paths && sample_nr >= min_paths) {
			struct pathspec_index_range *literal_ranges =
				pathspec_literal_index_ranges(repo->index, pathspec,
							      &ranges_nr);
	...
	if (!include_untracked &&
		...
		for (size_t i = 0;
		     i < (recursive_basename_pathspec ? 1 : pathspec->nr);
		     i++) {
			const struct pathspec_item *item = &pathspec->items[i];
			...
			} else if (!recursive_basename_pathspec &&
				   !wildcard_prefix) {
				end = nr + 1;
			}
		...
		if (worktree_cache && worktree_sidecar_loaded &&
			...
			if (refresh_bytes >= refresh_min_bytes) {
				...
				refresh_index(repo->index,
					      ...
					      pathspec, NULL, NULL);
				/*
				 * Save the refreshed fsmonitor bits so later greps do not
				 * repeat this scan. Failure to acquire the optional lock
				 * only loses the optimization.
				 */
				if (use_optional_locks() &&
				    repo_hold_locked_index(repo, &lock_file, 0) >= 0)
					repo_update_index_if_able(repo, &lock_file);
			}
		}
	}
	/*
...
struct grep_tree_query_context {
	...
	int tree_object_read_packed_unpack_invalid;
	int tree_object_read_packed_base_descent_invalid;
	...
	uint64_t tree_object_read_packed_unpack_attempt_count;
	uint64_t tree_object_read_packed_unpack_ns;
	uint64_t tree_object_read_packed_inflate_phase_count;
	uint64_t tree_object_read_packed_inflate_phase_ns;
	uint64_t tree_object_read_packed_base_descent_count;
...
static void grep_tree_record_packed_unpack(struct grep_tree_query_context *query,
					 const struct odb_read_result *result)
{
	uint64_t count, ns;

	if (query->tree_object_read_packed_unpack_invalid)
		return;
	if (query->tree_object_read_packed_content_invalid ||
	    result->packed_content_invalid || !result->packed_inflate_phase_enabled ||
	    result->packed_inflate_phase_invalid ||
		...
		goto invalid;
	/* This includes the cache-miss probe; derive it before rounding. */
	count = result->packed_content_attempt_count -
		result->packed_cache_copy_attempt_count;
	ns = result->packed_content_ns - result->packed_cache_copy_ns;
	if (result->packed_inflate_phase_ns > ns ||
	    (!count && result->packed_inflate_phase_count) ||
	    count > (uint64_t)INTMAX_MAX -
		query->tree_object_read_packed_unpack_attempt_count ||
	    ns > UINT64_MAX - query->tree_object_read_packed_unpack_ns ||
	    result->packed_inflate_phase_count > (uint64_t)INTMAX_MAX -
		query->tree_object_read_packed_inflate_phase_count ||
	    result->packed_inflate_phase_ns > UINT64_MAX -
		query->tree_object_read_packed_inflate_phase_ns)
		goto invalid;
	query->tree_object_read_packed_unpack_attempt_count += count;
	query->tree_object_read_packed_unpack_ns += ns;
	query->tree_object_read_packed_inflate_phase_count +=
		result->packed_inflate_phase_count;
	query->tree_object_read_packed_inflate_phase_ns +=
		result->packed_inflate_phase_ns;
	return;

invalid:
	query->tree_object_read_packed_unpack_invalid = 1;
}

static void grep_tree_record_packed_base_descent(struct grep_tree_query_context *query,
					       const struct odb_read_result *result)
{
	uint64_t unpack_count, unpack_ns;

	if (query->tree_object_read_packed_base_descent_invalid)
		return;
	if (query->tree_object_read_packed_content_invalid ||
	    result->packed_content_invalid || !result->packed_inflate_phase_enabled ||
	    result->packed_base_descent_invalid ||
	    result->packed_cache_copy_attempt_count >
		result->packed_content_attempt_count ||
	    result->packed_cache_copy_ns > result->packed_content_ns)
		goto invalid;
	unpack_count = result->packed_content_attempt_count -
		result->packed_cache_copy_attempt_count;
	unpack_ns = result->packed_content_ns - result->packed_cache_copy_ns;
	/* Check the disjoint raw intervals before independently rounding them. */
	if (result->packed_base_descent_count != unpack_count ||
	    result->packed_base_descent_ns > unpack_ns ||
	    (!result->packed_inflate_phase_invalid &&
	     result->packed_inflate_phase_ns > unpack_ns - result->packed_base_descent_ns) ||
	    result->packed_base_descent_count > (uint64_t)INTMAX_MAX -
		query->tree_object_read_packed_base_descent_count ||
	    result->packed_base_descent_ns > UINT64_MAX -
		query->tree_object_read_packed_base_descent_ns)
		goto invalid;
	query->tree_object_read_packed_base_descent_count +=
		result->packed_base_descent_count;
	query->tree_object_read_packed_base_descent_ns += result->packed_base_descent_ns;
	/* The checked parent sums also bound these monotonic subsets. */
...
invalid:
	query->tree_object_read_packed_base_descent_invalid = 1;
}
					...
					uint64_t elapsed)
{
	...
	grep_tree_record_packed_unpack(query, result);
	grep_tree_record_packed_base_descent(query, result);
	...
	const struct grep_tree_query_context *query)
{
	...
	int packed_unpack_valid = packed_content_valid &&
		!query->tree_object_read_packed_unpack_invalid;
	int packed_base_descent_valid = packed_content_valid &&
		!query->tree_object_read_packed_base_descent_invalid;
...
static int finish_grep_tree_batch(struct grep_tree_batch *batch)
{
	...
	for (size_t i = 0; i < buffer->nr; i++) {
		...
		if (oidset_contains(&query->impossible, &item->oid)) {
			result = GREP_INDEX_IPC_IMPOSSIBLE;
...
{
	/* Batch output is deferred, so drain it before replaying a fatal parse. */
...
static int grep_revision_index_maybe_activate(struct grep_revision_index *,
		     ...
		     struct grep_revision_index *revision_index)
{
	...
	while (tree->size) {
		...
		if (batch) {
			...
			next.flags |= TREE_DESC_SILENT_ERRORS;
			if (!tree_entry_gently(&next, &entry)) {
				flush_grep_tree_batch_before_die(batch);
				tree_entry(tree, &entry);
				BUG("tree_entry() returned after a gentle failure");
			}
			*tree = next;
		} else if (!tree_entry(tree, &entry)) {
			break;
		}
		...
		if (S_ISREG(entry.mode)) {
			...
			if (revision_index &&
			    grep_revision_index_maybe_activate(revision_index, base, tn_len) &&
			    grep_tree_from_matching_index(
				    opt, pathspec, &entry.oid, base, tn_len,
				    check_attr, batch, query, revision_index, &hit))
				goto next_entry;
			...
			strbuf_addch(base, '/');
			if (batch &&
			    init_tree_desc_gently(&sub, &entry.oid, data, size,
						  TREE_DESC_SILENT_ERRORS)) {
				flush_grep_tree_batch_before_die(batch);
				init_tree_desc(&sub, &entry.oid, data, size);
				BUG("init_tree_desc() returned after a gentle failure");
			} else if (!batch) {
				init_tree_desc(&sub, &entry.oid, data, size);
			}
			hit |= grep_tree(opt, pathspec, &sub, base, tn_len,
					 check_attr, batch, query, revision_index);
			free(data);
...
enum grep_revision_index_state {
	GREP_REVISION_INDEX_DISABLED,
	GREP_REVISION_INDEX_DEFERRED,
	GREP_REVISION_INDEX_ACTIVE,
};
...
static int grep_revision_index_load(struct repository *repo,
				    struct grep_revision_index *revision)
{
	...
	if (read_index_from_if_tree_accepted(
		    ...
		    grep_revision_index_accept, probe) < 0)
		goto fail;
	...
	struct grep_tree_query_context *query, struct grep_revision_index *revision)
{
	...
	return grep_revision_index_load(opt->repo, revision);
}

static int grep_revision_index_maybe_activate(struct grep_revision_index *revision,
					      const struct strbuf *base, int tn_len)
{
	size_t len = base->len - tn_len;

	if (revision->state == GREP_REVISION_INDEX_DEFERRED) {
		for (size_t i = 0; i < revision->paths.nr; i++) {
			const char *path = revision->paths.items[i].string;

			if (strlen(path) != len + 1 ||
			    memcmp(path, base->buf + tn_len, len))
				continue;
			return grep_revision_index_load(
				revision->index.repo, revision);
		}
	}
	return revision->state == GREP_REVISION_INDEX_ACTIVE;
}

/* A matching cache tree permits skipping child trees, as with cached diff. */
static int grep_tree_from_matching_index(
	struct grep_opt *opt, const struct pathspec *pathspec,
	...
	int *hit)
{
	...
	if (revision->window) {
		...
		strbuf_addstr(&prefix, base->buf + tn_len);
		strbuf_addch(&prefix, '/');
		pos = index_name_pos(index, prefix.buf, prefix.len);
		first = pos < 0 ? -pos - 1 : pos;
		count = entries;
		/* Validate the complete contiguous range before producing output. */
		if (count > index->cache_nr - first ||
		    !starts_with(index->cache[first]->name, prefix.buf) ||
		    !starts_with(index->cache[first + count - 1]->name, prefix.buf) ||
		    (first + count < index->cache_nr &&
		     starts_with(index->cache[first + count]->name, prefix.buf))) {
			strbuf_release(&prefix);
			return 0;
		}
		       ...
		       struct grep_tree_query_context *query)
{
	...
	if (obj->type == OBJ_COMMIT || obj->type == OBJ_TREE) {
		...
		batch.tree_name_len = base.len;
		batch.check_attr = obj->type == OBJ_COMMIT;
		if (content_index_query && !query->bypassed &&
		    !opt->status_only && opt->repo == the_repository &&
		    !recurse_submodules &&
		    fsm_settings__get_mode(opt->repo) == FSMONITOR_MODE_IPC) {
			if (query->ipc_available < 0)
				query->ipc_available =
					grep_index_ipc_is_available(opt->repo);
			if (query->ipc_available > 0) {
				batch.enabled = 1;
				batch_ptr = &batch;
			}
		}
		if (batch_ptr || threads_started) {
			if (init_tree_desc_gently(&tree, &obj->oid, data, size,
						  TREE_DESC_SILENT_ERRORS)) {
				if (threads_started)
					wait_all();
		...
		index.probe.tree_oid = &actual_tree_oid;
		index.probe.tree = tree;
		index.probe.paths = &index.paths;
		if ((!path || !*path) &&
		    grep_revision_index_prepare(opt, pathspec, query, &index))
			revision_index = &index;
		if (!revision_index || index.state != GREP_REVISION_INDEX_ACTIVE ||
		    !grep_tree_from_matching_index(opt, pathspec, &actual_tree_oid,
						   &base, base.len, obj->type == OBJ_COMMIT,
						   batch_ptr, query, revision_index, &hit))
			...
			const struct object_array *list)
{
	struct grep_tree_query_context query = {
		...
		.batch_max_bytes = git_env_ulong(
			"GIT_TEST_GREP_TREE_INDEX_BATCH_MAX_BYTES",
			GREP_TREE_INDEX_BATCH_MAX_BYTES),
	};
	unsigned int i;
	int hit = 0;
	int has_recursive_basename = 0;
	...
	if (!query.batch_size)
		...
		query.batch_size = GREP_TREE_INDEX_BATCH_SIZE;
	if (!query.batch_max_bytes ||
	    query.batch_max_bytes > GREP_TREE_INDEX_BATCH_MAX_BYTES)
		query.batch_max_bytes = GREP_TREE_INDEX_BATCH_MAX_BYTES;
	...
	if (query.objects) {
		...
		trace2_data_intmax("grep", the_repository,
				   "content_index_tree_queried", query.queried);
		trace2_data_intmax("grep", the_repository,
				   "content_index_tree_rejected", query.rejected);
		trace2_data_intmax("grep", the_repository,
